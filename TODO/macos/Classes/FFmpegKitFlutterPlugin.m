...
- (void)registerGlobalCallbacks {
  [FFmpegKitConfig
      enableFFmpegSessionCompleteCallback:^(FFmpegSession *session) {
        NSDictionary *dictionary =
            [FfmpegKitFlutterPlugin toSessionDictionary:session];
        dispatch_async(dispatch_get_main_queue(), ^() {
          self->_eventSink([FfmpegKitFlutterPlugin
  ...
  [FFmpegKitConfig enableStatisticsCallback:^(Statistics *statistics) {
    if (self->statisticsEnabled) {
      NSDictionary *dictionary =
          [FfmpegKitFlutterPlugin toStatisticsDictionary:statistics];
      dispatch_async(dispatch_get_main_queue(), ^() {
        self->_eventSink([FfmpegKitFlutterPlugin
...
- (void)mediaInformationJsonParserFrom:(NSString *)ffprobeJsonOutput
  ...
  @try {
    ...
    result(
        [FfmpegKitFlutterPlugin toMediaInformationDictionary:mediaInformation]);
...
- (void)getFFprobeSessions:(FlutterResult)result {
  result(
      [FfmpegKitFlutterPlugin toSessionArray:[FFprobeKit listFFprobeSessions]]);
}
...
+ (NSDictionary *)toSessionDictionary:(id<Session>)session {
  if (session != nil) {
    ...
    if ([session isFFmpeg]) {
      ...
      MediaInformationSession *mediaInformationSession =
          (MediaInformationSession *)session;
      dictionary[KEY_SESSION_MEDIA_INFORMATION] = [FfmpegKitFlutterPlugin
          toMediaInformationDictionary:[mediaInformationSession
                                           getMediaInformation]];
      dictionary[KEY_SESSION_TYPE] =
...
+ (NSArray *)toStatisticsArray:(NSArray *)statisticsArray {
  ...
  for (int i = 0; i < [statisticsArray count]; i++) {
    ...
    [array
        addObject:[FfmpegKitFlutterPlugin toStatisticsDictionary:statistics]];
  }
