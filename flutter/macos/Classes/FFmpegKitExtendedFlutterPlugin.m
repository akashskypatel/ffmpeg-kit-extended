...
static NSString *const EVENT_CHANNEL =
...
static NSString *const EVENT_STATISTICS_CALLBACK_EVENT =
    @"FFmpegKitStatisticsCallbackEvent";
static NSString *const EVENT_COMPLETE_CALLBACK_EVENT =
...
- (instancetype)init {
  ...
  if (self) {
    ...
    NSLog(@"FfmpegKitExtendedFlutterPlugin %p created.\n", self);
  }
...
- (FlutterError *)onListenWithArguments:(id)arguments
  ...
  NSLog(@"FfmpegKitExtendedFlutterPlugin %p started listening to events on %p.\n", self,
        eventSink);
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  FfmpegKitExtendedFlutterPlugin *instance = [[FfmpegKitExtendedFlutterPlugin alloc] init];
...
- (void)registerGlobalCallbacks {
  [FFmpegKitConfig
      enableFFmpegSessionCompleteCallback:^(FFmpegSession *session) {
        NSDictionary *dictionary =
            [FfmpegKitExtendedFlutterPlugin toSessionDictionary:session];
        dispatch_async(dispatch_get_main_queue(), ^() {
          self->_eventSink([FfmpegKitExtendedFlutterPlugin
              toStringDictionary:EVENT_COMPLETE_CALLBACK_EVENT
                  withDictionary:dictionary]);
        });
      }];

  [FFmpegKitConfig
      enableFFprobeSessionCompleteCallback:^(FFprobeSession *session) {
        NSDictionary *dictionary =
            [FfmpegKitExtendedFlutterPlugin toSessionDictionary:session];
        dispatch_async(dispatch_get_main_queue(), ^() {
          self->_eventSink([FfmpegKitExtendedFlutterPlugin
  ...
  [FFmpegKitConfig enableLogCallback:^(Log *log) {
    if (self->logsEnabled) {
      NSDictionary *dictionary = [FfmpegKitExtendedFlutterPlugin toLogDictionary:log];
      dispatch_async(dispatch_get_main_queue(), ^() {
        self->_eventSink([FfmpegKitExtendedFlutterPlugin
            toStringDictionary:EVENT_LOG_CALLBACK_EVENT
                withDictionary:dictionary]);
      });
    }
  }];
  ...
  [FFmpegKitConfig enableStatisticsCallback:^(Statistics *statistics) {
    if (self->statisticsEnabled) {
      NSDictionary *dictionary =
          [FfmpegKitExtendedFlutterPlugin toStatisticsDictionary:statistics];
      dispatch_async(dispatch_get_main_queue(), ^() {
        self->_eventSink([FfmpegKitExtendedFlutterPlugin
            toStringDictionary:EVENT_STATISTICS_CALLBACK_EVENT
                withDictionary:dictionary]);
      });
    }
  }];
...
- (void)handleMethodCall:(FlutterMethodCall *)call
  ...
  if ([@"abstractSessionGetEndTime" isEqualToString:call.method]) {
    if (sessionId != nil) {
      ...
      result([FlutterError errorWithCode:@"INVALID_SESSION"
    ...
    if (sessionId != nil) {
      [self abstractSessionGetAllLogs:sessionId
                              timeout:(NSNumber *)waitTimeout
                               result:result];
    } else {
      result([FlutterError errorWithCode:@"INVALID_SESSION"
                                 message:@"Invalid session id."
  ...
  } else if ([@"abstractSessionGetAllLogsAsString"
                 isEqualToString:call.method]) {
    if (sessionId != nil) {
      [self abstractSessionGetAllLogsAsString:sessionId
                                      timeout:(NSNumber *)waitTimeout
                                       result:result];
    } else {
      result([FlutterError errorWithCode:@"INVALID_SESSION"
  ...
  } else if ([@"abstractSessionGetFailStackTrace"
                 isEqualToString:call.method]) {
    if (sessionId != nil) {
  ...
  } else if ([@"thereAreAsynchronousMessagesInTransmit"
                 isEqualToString:call.method]) {
    if (sessionId != nil) {
    ...
    if (arguments != nil) {
      ...
      result([FlutterError errorWithCode:@"INVALID_ARGUMENTS"
                                 message:@"Invalid arguments array."
                                 details:nil]);
    }
  } else if ([@"ffmpegSessionGetAllStatistics" isEqualToString:call.method]) {
    if (sessionId != nil) {
      [self ffmpegSessionGetAllStatistics:sessionId
                                  timeout:(NSNumber *)waitTimeout
                                   result:result];
    } else {
      result([FlutterError errorWithCode:@"INVALID_SESSION"
    ...
    if (ffprobeJsonOutput != nil) {
      ...
      result([FlutterError errorWithCode:@"INVALID_FFPROBE_JSON_OUTPUT"
                                 message:@"Invalid ffprobe json output."
                                 details:nil]);
    }
  } else if ([@"mediaInformationJsonParserFromWithError"
                 isEqualToString:call.method]) {
    if (ffprobeJsonOutput != nil) {
      [self mediaInformationJsonParserFromWithError:ffprobeJsonOutput
                                             result:result];
    } else {
      result([FlutterError errorWithCode:@"INVALID_FFPROBE_JSON_OUTPUT"
                                 message:@"Invalid ffprobe json output."
    ...
    if (fontDirectory != nil) {
      ...
      result([FlutterError errorWithCode:@"INVALID_FONT_DIRECTORY"
                                 message:@"Invalid font directory."
                                 details:nil]);
    }
    ...
    if (fontDirectoryList != nil) {
      [self setFontDirectoryList:fontDirectoryList
                         mapping:fontNameMap
                          result:result];
    } else {
      result([FlutterError errorWithCode:@"INVALID_FONT_DIRECTORY_LIST"
                                 message:@"Invalid font directory list."
                                 details:nil]);
    }
    ...
    if (ffmpegPipePath != nil) {
      ...
      result([FlutterError errorWithCode:@"INVALID_PIPE_PATH"
                                 message:@"Invalid ffmpeg pipe path."
                                 details:nil]);
    }
    ...
    if ((variableName != nil) && (variableValue != nil)) {
      [self setEnvironmentVariable:variableName
                             value:variableValue
                            result:result];
    } else if (variableValue != nil) {
      result([FlutterError errorWithCode:@"INVALID_NAME"
                                 message:@"Invalid environment variable name."
                                 details:nil]);
    } else {
      result([FlutterError errorWithCode:@"INVALID_VALUE"
                                 message:@"Invalid environment variable value."
                                 details:nil]);
    }
    ...
    if (signalIndex != nil) {
      ...
      result([FlutterError errorWithCode:@"INVALID_SIGNAL"
                                 message:@"Invalid signal value."
                                 details:nil]);
    }
    ...
    if (sessionId != nil) {
      [self mediaInformationSessionExecute:sessionId
                                   timeout:waitTimeout
                                    result:result];
    } else {
      result([FlutterError errorWithCode:@"INVALID_SESSION"
  ...
  } else if ([@"asyncMediaInformationSessionExecute"
                 isEqualToString:call.method]) {
    if (sessionId != nil) {
      [self asyncMediaInformationSessionExecute:sessionId
                                        timeout:waitTimeout
                                         result:result];
    } else {
      result([FlutterError errorWithCode:@"INVALID_SESSION"
    ...
    if (level != nil) {
      ...
      result([FlutterError errorWithCode:@"INVALID_LEVEL"
    ...
    if (sessionHistorySize != nil) {
      ...
      result([FlutterError errorWithCode:@"INVALID_SIZE"
                                 message:@"Invalid session history size value."
                                 details:nil]);
    }
    ...
    if (stateIndex != nil) {
      ...
      result([FlutterError errorWithCode:@"INVALID_SESSION_STATE"
    ...
    if (strategyIndex != nil) {
      ...
      result([FlutterError
          errorWithCode:@"INVALID_LOG_REDIRECTION_STRATEGY"
                message:@"Invalid log redirection strategy value."
                details:nil]);
    }
    ...
    if ((input != nil) && (pipe != nil)) {
      ...
      result([FlutterError errorWithCode:@"INVALID_INPUT"
                                 message:@"Invalid input value."
                                 details:nil]);
    } else {
      result([FlutterError errorWithCode:@"INVALID_PIPE"
                                 message:@"Invalid pipe value."
                                 details:nil]);
    }
...
- (void)abstractSessionGetEndTime:(NSNumber *)sessionId
                           result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    result([FlutterError errorWithCode:@"SESSION_NOT_FOUND"
...
- (void)abstractSessionGetDuration:(NSNumber *)sessionId
                            result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
...
- (void)abstractSessionGetAllLogs:(NSNumber *)sessionId
                          timeout:(NSNumber *)waitTimeout
                           result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    ...
    int timeout;
    if ([FfmpegKitExtendedFlutterPlugin isValidPositiveNumber:waitTimeout]) {
    ...
    result([FfmpegKitExtendedFlutterPlugin toLogArray:allLogs]);
  }
}

- (void)abstractSessionGetLogs:(NSNumber *)sessionId
                        result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    ...
    result([FfmpegKitExtendedFlutterPlugin toLogArray:logs]);
  }
}

- (void)abstractSessionGetAllLogsAsString:(NSNumber *)sessionId
                                  timeout:(NSNumber *)waitTimeout
                                   result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
...
- (void)abstractSessionGetState:(NSNumber *)sessionId
                         result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    ...
    result([FfmpegKitExtendedFlutterPlugin sessionStateToNumber:[session getState]]);
  }
}

- (void)abstractSessionGetReturnCode:(NSNumber *)sessionId
                              result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
...
- (void)abstractSessionGetFailStackTrace:(NSNumber *)sessionId
                                  result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
...
- (void)thereAreAsynchronousMessagesInTransmit:(NSNumber *)sessionId
                                        result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    ...
    result([NSNumber
        numberWithBool:[session thereAreAsynchronousMessagesInTransmit]]);
  }
}
...
- (void)ffmpegSession:(NSArray *)arguments result:(FlutterResult)result {
  ...
  result([FfmpegKitExtendedFlutterPlugin toSessionDictionary:session]);
}

- (void)ffmpegSessionGetAllStatistics:(NSNumber *)sessionId
                              timeout:(NSNumber *)waitTimeout
                               result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    ...
    if ([session isFFmpeg]) {
      ...
      NSArray *allStatistics =
          [(FFmpegSession *)session getAllStatisticsWithTimeout:timeout];
      result([FfmpegKitExtendedFlutterPlugin toStatisticsArray:allStatistics]);
    } else {
      result([FlutterError
          errorWithCode:@"NOT_FFMPEG_SESSION"
                message:
...
- (void)ffmpegSessionGetStatistics:(NSNumber *)sessionId
                            result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    ...
    if ([session isFFmpeg]) {
      ...
      result([FfmpegKitExtendedFlutterPlugin toStatisticsArray:statistics]);
    } else {
      result([FlutterError
          errorWithCode:@"NOT_FFMPEG_SESSION"
                message:
...
- (void)ffprobeSession:(NSArray *)arguments result:(FlutterResult)result {
  FFprobeSession *session =
      [FFprobeSession create:arguments
                withCompleteCallback:nil
                     withLogCallback:nil
          withLogRedirectionStrategy:LogRedirectionStrategyNeverPrintLogs];
  result([FfmpegKitExtendedFlutterPlugin toSessionDictionary:session]);
...
- (void)mediaInformationSession:(NSArray *)arguments
                         result:(FlutterResult)result {
  ...
  if (session == nil) {
    ...
    if ([session isMediaInformation]) {
      MediaInformationSession *mediaInformationSession =
          (MediaInformationSession *)session;
      result([FfmpegKitExtendedFlutterPlugin
          toMediaInformationDictionary:[mediaInformationSession
                                           getMediaInformation]]);
    } else {
      result([FlutterError
          errorWithCode:@"NOT_MEDIA_INFORMATION_SESSION"
                message:
                    @"A session is found but it does not have the correct type."
                details:nil]);
    }
...
- (void)mediaInformationJsonParserFrom:(NSString *)ffprobeJsonOutput
                                result:(FlutterResult)result {
  @try {
    MediaInformation *mediaInformation =
        [MediaInformationJsonParser fromWithError:ffprobeJsonOutput];
    result(
        [FfmpegKitExtendedFlutterPlugin toMediaInformationDictionary:mediaInformation]);
...
- (void)mediaInformationJsonParserFromWithError:(NSString *)ffprobeJsonOutput
                                         result:(FlutterResult)result {
  @try {
    MediaInformation *mediaInformation =
        [MediaInformationJsonParser fromWithError:ffprobeJsonOutput];
    ...
    NSLog(@"Parsing MediaInformation failed: %@.\n",
          [NSString stringWithFormat:@"%@\n%@", [exception userInfo],
                                     [exception callStackSymbols]]);
    result([FlutterError
        errorWithCode:@"PARSE_FAILED"
              message:@"Parsing MediaInformation failed with JSON error."
              details:nil]);
  }
}
...
- (void)setFontDirectory:(NSString *)fontDirectoryPath
                 mapping:(NSDictionary *)fontNameMap
                  result:(FlutterResult)result {
...
- (void)setEnvironmentVariable:(NSString *)variableName
...
- (void)ignoreSignal:(int)signalIndex result:(FlutterResult)result {
  if ((signalIndex == 0) || (signalIndex == 1) || (signalIndex == 2) ||
    ...
    result([FlutterError errorWithCode:@"INVALID_SIGNAL"
                               message:@"Signal value not supported."
                               details:nil]);
  }
}

- (void)ffmpegSessionExecute:(NSNumber *)sessionId
                      result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
...
- (void)ffprobeSessionExecute:(NSNumber *)sessionId
                       result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    ...
    if ([session isFFprobe]) {
      ...
      result([FlutterError
          errorWithCode:@"NOT_FFPROBE_SESSION"
                message:
                    @"A session is found but it does not have the correct type."
                details:nil]);
    }
...
- (void)mediaInformationSessionExecute:(NSNumber *)sessionId
                               timeout:(NSNumber *)waitTimeout
                                result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    ...
    if ([session isMediaInformation]) {
      ...
      dispatch_async(asyncDispatchQueue, ^{
        [FFmpegKitConfig
            getMediaInformationExecute:(MediaInformationSession *)session
                           withTimeout:timeout];
        result(nil);
      });
    } else {
...
- (void)asyncFFmpegSessionExecute:(NSNumber *)sessionId
                           result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
...
- (void)asyncFFprobeSessionExecute:(NSNumber *)sessionId
                            result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
...
- (void)asyncMediaInformationSessionExecute:(NSNumber *)sessionId
                                    timeout:(NSNumber *)waitTimeout
                                     result:(FlutterResult)result {
  AbstractSession *session =
      (AbstractSession *)[FFmpegKitConfig getSession:[sessionId longValue]];
  if (session == nil) {
    ...
    if ([session isMediaInformation]) {
      ...
      [FFmpegKitConfig
          asyncGetMediaInformationExecute:(MediaInformationSession *)session
                              withTimeout:timeout];
      result(nil);
    } else {
      result([FlutterError
...
- (void)setSessionHistorySize:(NSNumber *)sessionHistorySize
                       result:(FlutterResult)result {
...
- (void)getLastSession:(FlutterResult)result {
  result([FfmpegKitExtendedFlutterPlugin
      toSessionDictionary:[FFmpegKitConfig getLastSession]]);
}
...
- (void)getSessions:(FlutterResult)result {
  result([FfmpegKitExtendedFlutterPlugin toSessionArray:[FFmpegKitConfig getSessions]]);
}
...
- (void)getSessionsByState:(int)stateIndex result:(FlutterResult)result {
  if ((stateIndex == 0) || (stateIndex == 1) || (stateIndex == 2) ||
    ...
    result([FfmpegKitExtendedFlutterPlugin
        toSessionArray:[FFmpegKitConfig getSessionsByState:sessionState]]);
  } else {
    result([FlutterError errorWithCode:@"INVALID_SESSION_STATE"
                               message:@"Session state value not supported."
                               details:nil]);
  }
}
...
- (void)getLogRedirectionStrategy:(FlutterResult)result {
  result([FfmpegKitExtendedFlutterPlugin
      logRedirectionStrategyToNumber:[FFmpegKitConfig
                                         getLogRedirectionStrategy]]);
}

- (void)setLogRedirectionStrategy:(int)strategyIndex
                           result:(FlutterResult)result {
  if ((strategyIndex == 0) || (strategyIndex == 1) || (strategyIndex == 2) ||
      (strategyIndex == 3) || (strategyIndex == 4)) {
    ...
    result([FlutterError
        errorWithCode:@"INVALID_LOG_REDIRECTION_STRATEGY"
              message:@"Log redirection strategy value not supported."
              details:nil]);
  }
}
...
- (void)messagesInTransmit:(NSNumber *)sessionId result:(FlutterResult)result {
  result(
      [NSNumber numberWithInt:[FFmpegKitConfig
                                  messagesInTransmit:[sessionId longValue]]]);
}
...
- (void)writeToPipe:(NSString *)inputPath
               pipe:(NSString *)namedPipePath
             result:(FlutterResult)result {
  dispatch_async(asyncDispatchQueue, ^{
    ...
    NSFileHandle *fileHandle =
        [NSFileHandle fileHandleForReadingAtPath:inputPath];
    if (fileHandle == nil) {
      NSLog(@"Failed to open file %@.\n", inputPath);
      result([FlutterError
          errorWithCode:@"COPY_FAILED"
                message:[NSString stringWithFormat:@"Failed to open file %@.",
                                                   inputPath]
                details:nil]);
      return;
    }
    ...
    NSFileHandle *pipeHandle =
        [NSFileHandle fileHandleForWritingAtPath:namedPipePath];
    if (pipeHandle == nil) {
      NSLog(@"Failed to open pipe %@.\n", namedPipePath);
      result([FlutterError
          errorWithCode:@"COPY_FAILED"
                message:[NSString stringWithFormat:@"Failed to open pipe %@.",
                                                   namedPipePath]
                details:nil]);
    ...
    @try {
      ...
      NSLog(@"Copying %@ to pipe %@ operation completed successfully. %lu "
            @"bytes copied in %f seconds.\n",
      ...
      result([FlutterError
          errorWithCode:@"COPY_FAILED"
                message:[NSString stringWithFormat:
                                      @"Copy %@ to %@ failed with error %@.",
                                      inputPath, namedPipePath, [e reason]]
                details:nil]);
    } @finally {
      [fileHandle closeFile];
...
- (void)selectDocument:(FlutterResult)result {
  result([FlutterError errorWithCode:@"NOT_SUPPORTED"
                             message:@"Not supported on macOS platform."
                             details:nil]);
}
...
- (void)getFFmpegSessions:(FlutterResult)result {
  result([FfmpegKitExtendedFlutterPlugin toSessionArray:[FFmpegKit listSessions]]);
}
...
- (void)getFFprobeSessions:(FlutterResult)result {
  result(
      [FfmpegKitExtendedFlutterPlugin toSessionArray:[FFprobeKit listFFprobeSessions]]);
}

- (void)getMediaInformationSessions:(FlutterResult)result {
  result([FfmpegKitExtendedFlutterPlugin
      toSessionArray:[FFprobeKit listMediaInformationSessions]]);
}
...
+ (NSDictionary *)toSessionDictionary:(id<Session>)session {
  if (session != nil) {
    ...
    dictionary[KEY_SESSION_ID] =
        [NSNumber numberWithLong:[session getSessionId]];
    dictionary[KEY_SESSION_CREATE_TIME] = [NSNumber
        numberWithLong:[[session getCreateTime] timeIntervalSince1970] * 1000];
    dictionary[KEY_SESSION_START_TIME] = [NSNumber
        numberWithLong:[[session getStartTime] timeIntervalSince1970] * 1000];
    dictionary[KEY_SESSION_COMMAND] = [session getCommand];

    if ([session isFFmpeg]) {
      dictionary[KEY_SESSION_TYPE] =
          [NSNumber numberWithInt:SESSION_TYPE_FFMPEG];
    } else if ([session isFFprobe]) {
      dictionary[KEY_SESSION_TYPE] =
          [NSNumber numberWithInt:SESSION_TYPE_FFPROBE];
    } else if ([session isMediaInformation]) {
      MediaInformationSession *mediaInformationSession =
          (MediaInformationSession *)session;
      dictionary[KEY_SESSION_MEDIA_INFORMATION] = [FfmpegKitExtendedFlutterPlugin
          toMediaInformationDictionary:[mediaInformationSession
                                           getMediaInformation]];
      dictionary[KEY_SESSION_TYPE] =
          [NSNumber numberWithInt:SESSION_TYPE_MEDIA_INFORMATION];
    }
...
+ (NSDictionary *)toLogDictionary:(Log *)log {
  if (log != nil) {
    ...
    dictionary[KEY_LOG_SESSION_ID] =
        [NSNumber numberWithLong:[log getSessionId]];
...
+ (NSDictionary *)toStatisticsDictionary:(Statistics *)statistics {
  if (statistics != nil) {
    ...
    dictionary[KEY_STATISTICS_SESSION_ID] =
        [NSNumber numberWithLong:[statistics getSessionId]];
    dictionary[KEY_STATISTICS_VIDEO_FRAME_NUMBER] =
        [NSNumber numberWithInt:[statistics getVideoFrameNumber]];
    dictionary[KEY_STATISTICS_VIDEO_FPS] =
        [NSNumber numberWithFloat:[statistics getVideoFps]];
    dictionary[KEY_STATISTICS_VIDEO_QUALITY] =
        [NSNumber numberWithFloat:[statistics getVideoQuality]];
    dictionary[KEY_STATISTICS_SIZE] =
        [NSNumber numberWithLong:[statistics getSize]];
    dictionary[KEY_STATISTICS_TIME] =
        [NSNumber numberWithDouble:[statistics getTime]];
    dictionary[KEY_STATISTICS_BITRATE] =
        [NSNumber numberWithDouble:[statistics getBitrate]];
    dictionary[KEY_STATISTICS_SPEED] =
        [NSNumber numberWithDouble:[statistics getSpeed]];
...
+ (NSDictionary *)toMediaInformationDictionary:
    (MediaInformation *)mediaInformation {
...
+ (NSArray *)toLogArray:(NSArray *)logs {
  ...
  for (int i = 0; i < [logs count]; i++) {
    ...
    [array addObject:[FfmpegKitExtendedFlutterPlugin toLogDictionary:log]];
  }
...
+ (NSArray *)toStatisticsArray:(NSArray *)statisticsArray {
  ...
  for (int i = 0; i < [statisticsArray count]; i++) {
    ...
    [array
        addObject:[FfmpegKitExtendedFlutterPlugin toStatisticsDictionary:statistics]];
  }
...
+ (NSArray *)toSessionArray:(NSArray *)sessions {
  ...
  for (int i = 0; i < [sessions count]; i++) {
    ...
    [array addObject:[FfmpegKitExtendedFlutterPlugin toSessionDictionary:session]];
  }

  return array;
}

+ (NSDictionary *)toStringDictionary:(NSString *)key
                      withDictionary:(NSDictionary *)value {
  NSMutableDictionary *dictionary = [[NSMutableDictionary alloc] init];
...
+ (NSNumber *)logRedirectionStrategyToNumber:
