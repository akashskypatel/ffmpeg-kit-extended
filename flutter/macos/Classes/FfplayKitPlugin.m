...
// ─── ffplay C API ─────────────────────────────────────────────────────────────
// ffplay_set_frame_callback is exported from the linked ffmpegkit framework.
...
typedef void (*FFplayFrameCb)(void *userdata, const uint8_t *pixels,
                               int width, int height, int linesize);
extern void ffplay_set_frame_callback(FFplayFrameCb callback, void *userdata);
...
- (void)_ensurePoolForWidth:(int)w height:(int)h {
    if (_pool && _poolWidth == w && _poolHeight == h) return;
...
- (void)updateWithPixels:(const uint8_t *)pixels
    ...
    if (_destroyed) { [_lock unlock]; return; }
    ...
    if (newBuf) {
        ...
        //   out[0]=B = src[2], out[1]=G = src[1], out[2]=R = src[0], out[3]=A = src[3]
        vImage_Buffer srcVBuf = {
            (void *)pixels,
            (vImagePixelCount)height, (vImagePixelCount)width, (size_t)linesize
        };
        vImage_Buffer dstVBuf = {
            dstBase,
            (vImagePixelCount)height, (vImagePixelCount)width, dstStride
        };
        const uint8_t permuteMap[4] = {2, 1, 0, 3};
        vImagePermuteChannels_ARGB8888(&srcVBuf, &dstVBuf, permuteMap, kvImageNoFlags);

        CVPixelBufferUnlockBaseAddress(newBuf, 0);

        if (_latestBuffer) CVPixelBufferRelease(_latestBuffer);
        _latestBuffer = newBuf;  // +1 refcount from CVPixelBufferPoolCreatePixelBuffer
    }
...
static void ffplay_frame_cb(void *userdata, const uint8_t *pixels,
                              int width, int height, int linesize) {
    if (!userdata || !pixels) return;
...
- (void)handleCreateTexture:(FlutterResult)result {
    ...
    tex.onFrameAvailable = ^{ [weakReg textureFrameAvailable:tid]; };
...
- (void)handleReleaseTexture:(FlutterMethodCall *)call result:(FlutterResult)result {
...
#pragma clang diagnostic ignored "-Wunused-variable"
        FfkitPixelTexture *tmp = (__bridge_transfer FfkitPixelTexture *)_retainedTexPtr;
