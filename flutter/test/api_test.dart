...
typedef NativeCompleteCallback =
    Void Function(Pointer<Void> session, Pointer<Void> userData);
...
extension ArenaTrackers on Arena {
  ...
  Pointer<Void> trackHandle(Pointer<Void> handle) {
    if (handle != nullptr) {
      this.using(handle, ffmpeg.ffmpeg_kit_handle_release);
    }
    return handle;
  }

  /// Tracks pointers allocated internally by the C library that must be freed with ffmpeg_kit_free.
  Pointer<T> trackKitPointer<T extends NativeType>(Pointer<T> ptr) {
    if (ptr != nullptr) {
      this.using(ptr.cast<Void>(), ffmpeg.ffmpeg_kit_free);
    }
    return ptr;
  }
...
void main() {
  ...
  setUp(
    () => using((Arena arena) {
      try {
        ...
        ffmpeg.ffmpeg_kit_config_clear_sessions();
        ffmpeg.ffmpeg_kit_config_set_log_level(
          ffmpeg.FFmpegKitLogLevel.FFMPEG_KIT_LOG_LEVEL_INFO,
        );
        ...
        ffmpeg.ffmpeg_kit_config_enable_statistics_callback(nullptr, nullptr);
  ...
  File generateTestVideoFile() => using((Arena arena) {
    ...
    final session = arena.trackHandle(ffmpeg.ffmpeg_kit_create_session(cmd));
    ffmpeg.ffmpeg_kit_session_execute(session);
  ...
  Future<void> waitForSessionToSettle(
    Pointer<Void> session, {
    int timeoutMs = 10000,
  }) async {
    final start = DateTime.now();
    final sessionId = ffmpeg.ffmpeg_kit_session_get_session_id(session);

    while (DateTime.now().difference(start).inMilliseconds < timeoutMs) {
      final state = ffmpeg.ffmpeg_kit_session_get_state(session);

      // 0 = no messages in transit, everything is flushed
      final messagesInTransmit = ffmpeg.ffmpeg_kit_config_messages_in_transmit(
        sessionId,
      );

      final isTerminal =
          state ==
              ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_COMPLETED ||
          state == ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_FAILED;
  ...
  group('FFmpegKit C API Pairity Tests', () {
    ...
    test('FFmpegKitTest VersionCheck', () {
      using((Arena arena) {
        ...
        final Pointer<Char> versionPtr = ffmpeg
        ...
        ffmpeg.ffmpeg_kit_free(versionPtr.cast());
      });
    });
    ...
    test('FFmpegKitTest SplitSessionExecution', () {
      using((Arena arena) {
        ...
        var state = ffmpeg.ffmpeg_kit_session_get_state(session);
        expect(
          state,
          equals(ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_CREATED),
        );

        // ffmpeg_kit_session_execute(session);
        ffmpeg.ffmpeg_kit_session_execute(session);

        state = ffmpeg.ffmpeg_kit_session_get_state(session);

        if (state !=
            ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_COMPLETED) {
          final returnCode = ffmpeg.ffmpeg_kit_session_get_return_code(session);
          if (kDebugMode) print("Return Code: $returnCode");

          final logsPtr = ffmpeg.ffmpeg_kit_session_get_logs_as_string(session);
          if (logsPtr != nullptr) {
            if (kDebugMode) print("Logs:\n${_fromNative(logsPtr)}");
            ffmpeg.ffmpeg_kit_free(logsPtr.cast());
          }

          final failStackTracePtr = ffmpeg
              .ffmpeg_kit_session_get_fail_stack_trace(session);
    ...
    test('FFmpegKitTest DebugLog', () {
      using((Arena arena) {
        ...
        ffmpeg.ffmpeg_kit_config_enable_debug_log(session);
        expect(ffmpeg.ffmpeg_kit_config_is_debug_log_enabled(session), isTrue);
        ...
        final debugLogPtr = ffmpeg.ffmpeg_kit_config_get_debug_log(session);
        expect(debugLogPtr, isNot(nullptr));
        if (kDebugMode) print("Debug Log:\n${_fromNative(debugLogPtr)}");
        ffmpeg.ffmpeg_kit_free(debugLogPtr.cast());

        // ffmpeg_kit_config_disable_debug_log(session);
        ffmpeg.ffmpeg_kit_config_disable_debug_log(session);
        expect(ffmpeg.ffmpeg_kit_config_is_debug_log_enabled(session), isFalse);

        // ffmpeg_kit_config_clear_debug_log(session);
        ffmpeg.ffmpeg_kit_config_clear_debug_log(session);
        final debugLogPtr2 = ffmpeg.ffmpeg_kit_config_get_debug_log(session);
        expect(_fromNative(debugLogPtr2), isEmpty);
        ffmpeg.ffmpeg_kit_free(debugLogPtr2.cast());
    ...
    test('FFmpegKitTest ConfigurationSetters', () {
      using((Arena arena) {
        ...
        ffmpeg.ffmpeg_kit_config_set_log_level(
          ffmpeg.FFmpegKitLogLevel.FFMPEG_KIT_LOG_LEVEL_QUIET,
        );

        // EXPECT_EQ(ffmpeg_kit_config_get_log_level(), FFMPEG_KIT_LOG_LEVEL_QUIET);
        final level = ffmpeg.ffmpeg_kit_config_get_log_level();
        expect(
          level,
          equals(ffmpeg.FFmpegKitLogLevel.FFMPEG_KIT_LOG_LEVEL_QUIET),
        );
      });
    });
    ...
    test('FFmpegKitTest SessionHistory', () {
      using((Arena arena) {
        ...
        ffmpeg.ffmpeg_kit_set_session_history_size(10);

        // int history_size = ffmpeg_kit_get_session_history_size();
        final historySize = ffmpeg.ffmpeg_kit_get_session_history_size();
        if (kDebugMode) print("History size: $historySize");
        expect(historySize, equals(10));

        // Get initial count to compare later
        int initialCount = 0;
        final Pointer<ffmpeg.FFmpegSessionHandle> initialSessionsPtr = ffmpeg
        ...
        if (initialSessionsPtr != nullptr) {
          ...
          while (list[initialCount] != nullptr) {
            ffmpeg.ffmpeg_kit_handle_release(list[initialCount]);
            initialCount++;
          }
          ffmpeg.ffmpeg_kit_free(initialSessionsPtr.cast());
        }
        ...
        final cmd = "-hide_banner -loglevel fatal -version"
            ...
            .cast<Char>();
        for (int i = 0; i < 3; i++) {
          final s = ffmpeg.ffmpeg_kit_create_session(cmd);
          ffmpeg.ffmpeg_kit_session_execute(s);
          ffmpeg.ffmpeg_kit_handle_release(s);
        }

        // FFmpegSessionHandle *sessions = ffmpeg_kit_get_sessions();
        final Pointer<ffmpeg.FFmpegSessionHandle> sessionsPtr = ffmpeg
            .ffmpeg_kit_get_sessions();
        ...
        while (list[count] != nullptr) {
          ...
          ffmpeg.ffmpeg_kit_handle_release(list[count]);
          count++;
        }

        // free(sessions);
        ffmpeg.ffmpeg_kit_free(sessionsPtr.cast());
    ...
    test('FFmpegKitTest MediaInformation', () {
      ...
      using((Arena arena) {
        ...
        final mediaSession = ffmpeg.ffprobe_kit_get_media_information(pathPtr);
        ...
        final state = ffmpeg.ffmpeg_kit_session_get_state(mediaSession);

        if (state !=
            ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_COMPLETED) {
          final logsPtr = ffmpeg.ffmpeg_kit_session_get_logs_as_string(
            mediaSession,
          );
          if (logsPtr != nullptr) {
            if (kDebugMode) print("Logs:\n${_fromNative(logsPtr)}");
        ...
        final createTime = ffmpeg.ffmpeg_kit_session_get_create_time(
          mediaSession,
        );
        expect(createTime, greaterThan(0));

        // char *cmd = ffmpeg_kit_session_get_command(media_session);
        final cmdPtr = ffmpeg.ffmpeg_kit_session_get_command(mediaSession);
        ...
        final logCount = ffmpeg.ffmpeg_kit_session_get_logs_count(mediaSession);
        ...
        final info = ffmpeg.media_information_session_get_media_information(
          mediaSession,
        );
        expect(info, isNot(nullptr));

        // char *filename = media_information_get_filename(info);
        final filenamePtr = ffmpeg.media_information_get_filename(info);
        expect(filenamePtr, isNot(nullptr));
        ffmpeg.ffmpeg_kit_free(filenamePtr.cast());

        // char *duration = media_information_get_duration(info);
        final durationPtr = ffmpeg.media_information_get_duration(info);
        expect(durationPtr, isNot(nullptr));
        ffmpeg.ffmpeg_kit_free(durationPtr.cast());

        // char *bitrate = media_information_get_bitrate(info);
        final bitratePtr = ffmpeg.media_information_get_bitrate(info);
        expect(bitratePtr, isNot(nullptr));
        ffmpeg.ffmpeg_kit_free(bitratePtr.cast());

        // char *size = media_information_get_size(info);
        final sizePtr = ffmpeg.media_information_get_size(info);
        expect(sizePtr, isNot(nullptr));
        ffmpeg.ffmpeg_kit_free(sizePtr.cast());

        // int streams_count = media_information_get_streams_count(info);
        final streamsCount = ffmpeg.media_information_get_streams_count(info);
        expect(streamsCount, greaterThanOrEqualTo(1));

        if (streamsCount > 0) {
          // StreamInformationHandle stream = media_information_get_stream_at(info, 0);
          final stream = ffmpeg.media_information_get_stream_at(info, 0);
          expect(stream, isNot(nullptr));

          // long index = stream_information_get_index(stream);
          expect(
            ffmpeg.stream_information_get_index(stream),
            greaterThanOrEqualTo(0),
          );

          final typePtr = ffmpeg.stream_information_get_type(stream);
          if (typePtr != nullptr) ffmpeg.ffmpeg_kit_free(typePtr.cast());

          final codecPtr = ffmpeg.stream_information_get_codec(stream);
          if (codecPtr != nullptr) ffmpeg.ffmpeg_kit_free(codecPtr.cast());

          final codecLongPtr = ffmpeg.stream_information_get_codec_long(stream);
          if (codecLongPtr != nullptr) {
            ffmpeg.ffmpeg_kit_free(codecLongPtr.cast());
          }

          final formatPtr = ffmpeg.stream_information_get_format(stream);
          if (formatPtr != nullptr) ffmpeg.ffmpeg_kit_free(formatPtr.cast());

          final bitrateSPtr = ffmpeg.stream_information_get_bitrate(stream);
          if (bitrateSPtr != nullptr) {
            ffmpeg.ffmpeg_kit_free(bitrateSPtr.cast());
          }

          final sampleRatePtr = ffmpeg.stream_information_get_sample_rate(
            stream,
          );
          if (sampleRatePtr != nullptr) {
            ffmpeg.ffmpeg_kit_free(sampleRatePtr.cast());
          }

          expect(
            ffmpeg.stream_information_get_width(stream),
            greaterThanOrEqualTo(0),
          );
          expect(
            ffmpeg.stream_information_get_height(stream),
            greaterThanOrEqualTo(0),
          );

          final tagsPtr = ffmpeg.stream_information_get_tags_json(stream);
          if (tagsPtr != nullptr) ffmpeg.ffmpeg_kit_free(tagsPtr.cast());

          final sampleFormatPtr = ffmpeg.stream_information_get_sample_format(
            stream,
          );
          if (sampleFormatPtr != nullptr) {
            ffmpeg.ffmpeg_kit_free(sampleFormatPtr.cast());
          }

          final darPtr = ffmpeg.stream_information_get_display_aspect_ratio(
            stream,
          );
          if (darPtr != nullptr) ffmpeg.ffmpeg_kit_free(darPtr.cast());

          final afrPtr = ffmpeg.stream_information_get_average_frame_rate(
            stream,
          );
          if (afrPtr != nullptr) ffmpeg.ffmpeg_kit_free(afrPtr.cast());

          final rfrPtr = ffmpeg.stream_information_get_real_frame_rate(stream);
          if (rfrPtr != nullptr) ffmpeg.ffmpeg_kit_free(rfrPtr.cast());

          final tbPtr = ffmpeg.stream_information_get_time_base(stream);
          if (tbPtr != nullptr) ffmpeg.ffmpeg_kit_free(tbPtr.cast());

          final clPtr = ffmpeg.stream_information_get_channel_layout(stream);
          if (clPtr != nullptr) ffmpeg.ffmpeg_kit_free(clPtr.cast());

          final sarPtr = ffmpeg.stream_information_get_sample_aspect_ratio(
            stream,
          );
          if (sarPtr != nullptr) ffmpeg.ffmpeg_kit_free(sarPtr.cast());

          final ctbPtr = ffmpeg.stream_information_get_codec_time_base(stream);
          if (ctbPtr != nullptr) ffmpeg.ffmpeg_kit_free(ctbPtr.cast());

          final stringPropPtr = ffmpeg.stream_information_get_string_property(
            stream,
            toNative("codec_name", arena),
          );
          if (stringPropPtr != nullptr) {
            ffmpeg.ffmpeg_kit_free(stringPropPtr.cast());
          }

          expect(
            ffmpeg.stream_information_get_number_property(
              stream,
              toNative("index", arena),
            ),
            greaterThanOrEqualTo(0),
          );

          final allStreamPropsPtr = ffmpeg
              .stream_information_get_all_properties_json(stream);
          if (allStreamPropsPtr != nullptr) {
            ffmpeg.ffmpeg_kit_free(allStreamPropsPtr.cast());
          }

          ffmpeg.ffmpeg_kit_handle_release(stream);
        }

        final chaptersCount = ffmpeg.media_information_get_chapters_count(info);
        expect(chaptersCount, greaterThanOrEqualTo(0));

        if (chaptersCount > 0) {
          final chapter = ffmpeg.media_information_get_chapter_at(info, 0);
          expect(chapter, isNot(nullptr));
          expect(ffmpeg.chapter_get_id(chapter), greaterThanOrEqualTo(0));

          final startTimePtr = ffmpeg.chapter_get_start_time(chapter);
          if (startTimePtr != nullptr) {
            ffmpeg.ffmpeg_kit_free(startTimePtr.cast());
          }

          final endTimePtr = ffmpeg.chapter_get_end_time(chapter);
          if (endTimePtr != nullptr) {
            ffmpeg.ffmpeg_kit_free(endTimePtr.cast());
          }

          ffmpeg.ffmpeg_kit_handle_release(chapter);
        }

        final allInfoPropsPtr = ffmpeg
            .media_information_get_all_properties_json(info);
        expect(allInfoPropsPtr, isNot(nullptr));
        ffmpeg.ffmpeg_kit_free(allInfoPropsPtr.cast());

        ffmpeg.ffmpeg_kit_handle_release(info);
        ffmpeg.ffmpeg_kit_handle_release(mediaSession);
      });
    });
    test('FFmpegKitTest MediaInformationSessionAPIs', () async {
      ...
      await using((Arena arena) async {
        ...
        final session = ffmpeg.media_information_create_session(cmd);
        if (kDebugMode) print("Media Information Session: $session");
        expect(session, isNot(nullptr));

        // media_information_session_execute_async(session, 1000);
        ffmpeg.media_information_session_execute_async(session, 1000);
    ...
    test('FFplayKitTest FFplaySession', () async {
      ...
      await using((Arena arena) async {
        ...
        ffmpeg.ffmpeg_kit_config_set_environment_variable(
          toNative("SDL_VIDEODRIVER", arena),
          toNative("dummy", arena),
        );
        ffmpeg.ffmpeg_kit_config_set_environment_variable(
          toNative("SDL_AUDIODRIVER", arena),
          toNative("dummy", arena),
        );
        ffmpeg.ffmpeg_kit_config_set_environment_variable(
          toNative("DISPLAY", arena),
          toNative(":0", arena),
        );
        ...
        final playSession = ffmpeg.ffplay_kit_execute(cmd, 1000);
        ...
        final state = ffmpeg.ffmpeg_kit_session_get_state(playSession);
        ...
        final returnCode = ffmpeg.ffmpeg_kit_session_get_return_code(
          playSession,
        );
        if (kDebugMode) print("Return Code: $returnCode");
        ...
        // Cleanup
        ffmpeg.ffmpeg_kit_handle_release(playSession);
      });
    });
    ...
    test('FFplayKitInteractiveTest PlayPauseResume', () async {
      ...
      await using((Arena arena) async {
        ...
        final session = ffmpeg.ffplay_kit_execute_async(
          cmd,
          nullptr,
          nullptr,
          1000,
        ...
        final isPlaying = ffmpeg.ffplay_kit_session_is_playing(session);
        if (kDebugMode) print("Is Playing: $isPlaying");
        expect(isPlaying, isTrue);

        // ffplay_kit_session_pause(session);
        ffmpeg.ffplay_kit_session_pause(session);
        await Future.delayed(const Duration(seconds: 1));

        // EXPECT_EQ(ffplay_kit_session_is_paused(session), 1);
        final isPaused = ffmpeg.ffplay_kit_session_is_paused(session);
        if (kDebugMode) print("Is Paused: $isPaused");
        expect(isPaused, isTrue);

        // ffplay_kit_session_resume(session);
        ffmpeg.ffplay_kit_session_resume(session);
        await Future.delayed(const Duration(seconds: 1));

        // EXPECT_EQ(ffplay_kit_session_is_paused(session), 0);
        expect(ffmpeg.ffplay_kit_session_is_paused(session), isFalse);

        // EXPECT_EQ(ffplay_kit_session_is_playing(session), 1);
        expect(ffmpeg.ffplay_kit_session_is_playing(session), isTrue);
    ...
    test('FFplayKitInteractiveTest Seek', () async {
      ...
      await using((Arena arena) async {
        ...
        // Seek Absolute
        ffmpeg.ffplay_kit_session_seek(session, 10.0);
        await Future.delayed(const Duration(seconds: 1));

        final double pos = ffmpeg.ffplay_kit_session_get_position(session);
        if (kDebugMode) print("Position: $pos");
        expect(pos, greaterThanOrEqualTo(5.0));

        // Seek Relative Backward
        ffmpeg.ffplay_kit_session_seek(session, -5.0);
        await Future.delayed(const Duration(seconds: 1));

        final double newPos = ffmpeg.ffplay_kit_session_get_position(session);
        if (kDebugMode) print("New Position: $newPos");
        expect(newPos, lessThan(pos));

        ffmpeg.ffmpeg_kit_handle_release(session.cast<Void>());
      });
    });
    ...
    test('FFplayKitInteractiveTest ConcurrentSessions', () async {
      ...
      await using((Arena arena) async {
        ...
        // Start Session 1
        final session1 = ffmpeg.ffplay_kit_execute_async(
          cmd,
          nullptr,
          nullptr,
          1000,
        ...
        final session2 = ffmpeg.ffplay_kit_execute_async(
          cmd,
          nullptr,
          nullptr,
          1000,
        ...
        final state1 = ffmpeg.ffmpeg_kit_session_get_state(session1);
        ...
        final isPlaying2 = ffmpeg.ffplay_kit_session_is_playing(session2);
    ...
    test('FFplayKitInteractiveTest GlobalControls', () async {
      ...
      await using((Arena arena) async {
        ...
        // ffplay_kit_pause();
        ffmpeg.ffplay_kit_pause();
        await Future.delayed(const Duration(seconds: 1));
        final isPaused = ffmpeg.ffplay_kit_is_paused();
        if (kDebugMode) print("Is Paused: $isPaused");
        expect(isPaused, isTrue);

        // ffplay_kit_resume();
        ffmpeg.ffplay_kit_resume();
        await Future.delayed(const Duration(seconds: 1));
        final isPausedAfterResume = ffmpeg.ffplay_kit_is_paused();
    ...
    test('FFplayKitInteractiveTest GlobalSeek', () async {
      ...
      await using((Arena arena) async {
        ...
        ffmpeg.ffplay_kit_set_position(10.0);
        await Future.delayed(const Duration(seconds: 1));

        // double pos = ffplay_kit_get_position();
        final double pos = ffmpeg.ffplay_kit_get_position();
        if (kDebugMode) print("Position: $pos");
        expect(pos, greaterThanOrEqualTo(9.0));

        // Global Seek (Relative)
        // ffplay_kit_seek(-5.0);
        ffmpeg.ffplay_kit_seek(-5.0);
        await Future.delayed(const Duration(seconds: 1));

        final double newPos = ffmpeg.ffplay_kit_get_position();
        if (kDebugMode) print("New Position: $newPos");
        expect(newPos, lessThan(pos));
    ...
    test('FFplayKitInteractiveTest SessionAPIs', () async {
      ...
      await using((Arena arena) async {
        ...
        ffmpeg.ffplay_kit_session_close(session);

        // Create and manual start test (Session 2)
        final session2 = ffmpeg.ffplay_kit_create_session(cmd);
        if (kDebugMode) print("Session2: $session2");
        expect(session2, isNot(nullptr));

        ffmpeg.ffplay_kit_session_execute_async(session2, 1000);
        await Future.delayed(const Duration(seconds: 2));

        ffmpeg.ffplay_kit_session_pause(session2);
        await Future.delayed(const Duration(seconds: 1));

        ffmpeg.ffplay_kit_session_resume(session2);
        await Future.delayed(const Duration(seconds: 1));
    ...
    test('FFplayKitInteractiveTest GlobalAPIs', () async {
      ...
      await using((Arena arena) async {
        ...
        ffmpeg.ffplay_kit_set_volume(0.5);
        await Future.delayed(const Duration(seconds: 1));

        final volume = ffmpeg.ffplay_kit_get_volume();
        if (kDebugMode) print("Volume: $volume");
        expect(volume, closeTo(0.5, 0.01));

        // printf("Duration: %f\n", ffplay_kit_get_duration());
        final duration = ffmpeg.ffplay_kit_get_duration();
        if (kDebugMode) print("Duration: $duration");
        expect(duration, greaterThan(0.0));

        // ffplay_kit_start();
        ffmpeg.ffplay_kit_start();
        await Future.delayed(const Duration(seconds: 1));
    ...
    test('FFplayKitInteractiveTest TimeoutSession', () async {
      ...
      await using((Arena arena) async {
        ...
        final isPlaying1 = ffmpeg.ffplay_kit_session_is_playing(session1);
    ...
    test('FFmpegKitTest PackageName', () async {
      await using((Arena arena) async {
        ...
        final pkgPtr = ffmpeg.ffmpeg_kit_packages_get_package_name();
        ...
        // free(pkg);
        ffmpeg.ffmpeg_kit_free(pkgPtr.cast());
      });
    });
    ...
    test('FFmpegKitTest AudioDeviceManagement', () async {
      ...
      await using((Arena arena) async {
        ...
        // 1. List devices
        final devicesPtr = ffmpeg.ffmpeg_kit_config_list_audio_output_devices();
        if (devicesPtr != nullptr) {
          if (kDebugMode) print("Audio Devices: ${_fromNative(devicesPtr)}");
          ffmpeg.ffmpeg_kit_free(devicesPtr.cast());
        }

        // 2. Set Device (API Verification)
        ffmpeg.ffmpeg_kit_config_set_audio_output_device(
          toNative("Test Device", arena),
        );

        // Reset to default before playback
        ffmpeg.ffmpeg_kit_config_set_audio_output_device(nullptr);
        ...
        if (await File(testAudioFile).exists()) {
          ...
          final session = ffmpeg.ffplay_kit_execute(cmd, 2000);
    ...
    test('FFmpegKitTest ConcurrentOperations', () async {
      ...
      await using((Arena arena) async {
        ...
        final ffmpegSession = ffmpeg.ffmpeg_kit_create_session(ffmpegCmd);
        ...
        final ffprobeSession = ffmpeg.ffprobe_kit_create_session(ffprobeCmd);
        if (kDebugMode) print("FFprobe Session: $ffprobeSession");
        expect(ffprobeSession, isNot(nullptr));

        // 3. Execute both asynchronously
        ffmpeg.ffmpeg_kit_session_execute_async(ffmpegSession);
        ffmpeg.ffprobe_kit_session_execute_async(ffprobeSession);

        // 4. Wait for both to complete
        int totalWait = 0;
        while (totalWait < 10000) {
          // 10s max
          final state1 = ffmpeg.ffmpeg_kit_session_get_state(ffmpegSession);
          final state2 = ffmpeg.ffmpeg_kit_session_get_state(ffprobeSession);
        ...
        // 5. Verify results
        final finalState1 = ffmpeg.ffmpeg_kit_session_get_state(ffmpegSession);
        final finalState2 = ffmpeg.ffmpeg_kit_session_get_state(ffprobeSession);
        ...
        // Cleanup native handles
        ffmpeg.ffmpeg_kit_handle_release(ffmpegSession);
        ffmpeg.ffmpeg_kit_handle_release(ffprobeSession);
    ...
    test('FFmpegKitTest ConcurrentFFmpegSessions', () async {
      await using((Arena arena) async {
        ...
        final ffmpegSession1 = ffmpeg.ffmpeg_kit_create_session(cmd1);
        final ffmpegSession2 = ffmpeg.ffmpeg_kit_create_session(cmd2);
        ...
        ffmpeg.ffmpeg_kit_session_execute_async(ffmpegSession1);
        ffmpeg.ffmpeg_kit_session_execute_async(ffmpegSession2);

        // 3. Wait for both to complete
        int totalWait = 0;
        while (totalWait < 10000) {
          final state1 = ffmpeg.ffmpeg_kit_session_get_state(ffmpegSession1);
          final state2 = ffmpeg.ffmpeg_kit_session_get_state(ffmpegSession2);
        ...
        // 4. Verify results
        final finalState1 = ffmpeg.ffmpeg_kit_session_get_state(ffmpegSession1);
        final finalState2 = ffmpeg.ffmpeg_kit_session_get_state(ffmpegSession2);
        ...
        // Cleanup
        ffmpeg.ffmpeg_kit_handle_release(ffmpegSession1);
        ffmpeg.ffmpeg_kit_handle_release(ffmpegSession2);
    ...
    test(
      ...
      () => using((Arena arena) async {
        ...
        final s1 = arena.trackHandle(ffmpeg.ffmpeg_kit_create_session(cmd1));

        if (kDebugMode) print("Session 1: $s1");
        expect(s1, isNotNull);
        ffmpeg.ffmpeg_kit_session_execute_async(s1);

        final startTime = DateTime.now();
        bool done = false;
        while (!done) {
          final st1 = ffmpeg.ffmpeg_kit_session_get_state(s1);
        ...
        if (kDebugMode) {
          print("Session 1 state: ${ffmpeg.ffmpeg_kit_session_get_state(s1)}");
        }
        expect(
          ffmpeg.ffmpeg_kit_session_get_state(s1),
          equals(
            ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_COMPLETED,
          ),
        );
    ...
    test('FFplayKitInteractiveTest FFplayWithFFmpegConcurrency', () async {
      ...
      await using((Arena arena) async {
        ...
        final playSession = ffmpeg.ffplay_kit_execute_async(
          playCmd,
          nullptr,
          nullptr,
          1000,
        ...
        final isPlaying = ffmpeg.ffplay_kit_session_is_playing(playSession);
        if (kDebugMode) print("FFplay Session Is Playing: $isPlaying");
        expect(isPlaying, isTrue);

        final ffmpegState = ffmpeg.ffmpeg_kit_session_get_state(ffmpegSession);
        ...
        int waitTotal = 0;
        while (ffmpeg.ffmpeg_kit_session_get_state(ffmpegSession) ==
    ...
    test('FFplayKitInteractiveTest FFplayWithFFprobeConcurrency', () async {
      ...
      await using((Arena arena) async {
        ...
        final probeCmd = toNative(
          "-hide_banner -loglevel fatal -show_format -i $videoFile",
          arena,
        );
        final probeSession = ffmpeg.ffprobe_kit_execute(probeCmd);
        ...
        final probeState = ffmpeg.ffmpeg_kit_session_get_state(probeSession);
        ...
        ffmpeg.ffmpeg_kit_handle_release(playSession);
        ffmpeg.ffmpeg_kit_handle_release(probeSession);
      });
    });
    test('FFmpegKitTest ConcurrentFFprobeSessions', () async {
      ...
      await using((Arena arena) async {
        ...
        final ffprobeSession1 = ffmpeg.ffprobe_kit_create_session(cmd);
        final ffprobeSession2 = ffmpeg.ffprobe_kit_create_session(cmd);

        expect(ffprobeSession1, isNot(nullptr));
        expect(ffprobeSession2, isNot(nullptr));

        // Attach callbacks to the sessions
        ffmpeg.ffprobe_kit_set_complete_callback(
          ffprobeSession1,
        ...
        // 2. Execute both asynchronously
        ffmpeg.ffprobe_kit_session_execute_async(ffprobeSession1);
        ffmpeg.ffprobe_kit_session_execute_async(ffprobeSession2);
        ...
        // 4. Verify results
        expect(
          ffmpeg.ffmpeg_kit_session_get_state(ffprobeSession1),
          equals(
            ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_COMPLETED,
          ),
        );
        expect(
          ffmpeg.ffmpeg_kit_session_get_state(ffprobeSession2),
          equals(
            ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_COMPLETED,
          ),
        );
        ...
        ffmpeg.ffmpeg_kit_handle_release(ffprobeSession1);
        ffmpeg.ffmpeg_kit_handle_release(ffprobeSession2);
      });
    });
    test('FFmpegKitTest SessionManagement', () async {
      ...
      await using((Arena arena) async {
        ...
        final ffmpegSession = ffmpeg.ffmpeg_kit_create_session(
          toNative("-version", arena),
        );
        final ffprobe = ffmpeg.ffprobe_kit_create_session(
          toNative("-version", arena),
        );
        final ffplay = ffmpeg.ffplay_kit_create_session(
          toNative("-version", arena),
        );

        final mediaCmd =
            "-v error -hide_banner -print_format json -show_format -show_streams -show_chapters -i ${getTestVideoFile()}";
        final media = ffmpeg.media_information_create_session(
          toNative(mediaCmd, arena),
        );

        // 3. Check last session
        final last = ffmpeg.ffmpeg_kit_get_last_session();
        if (kDebugMode) print("Last Session: $last");
        expect(last, isNot(nullptr));
        ffmpeg.ffmpeg_kit_handle_release(last);

        final lastFFmpeg = ffmpeg.ffmpeg_kit_get_last_ffmpeg_session();
        if (kDebugMode) print("Last FFmpeg Session: $lastFFmpeg");
        expect(lastFFmpeg, isNot(nullptr));
        ffmpeg.ffmpeg_kit_handle_release(lastFFmpeg);

        final lastFFprobe = ffmpeg.ffmpeg_kit_get_last_ffprobe_session();
        if (kDebugMode) print("Last FFprobe Session: $lastFFprobe");
        expect(lastFFprobe, isNot(nullptr));
        ffmpeg.ffmpeg_kit_handle_release(lastFFprobe);

        final lastFFplay = ffmpeg.ffmpeg_kit_get_last_ffplay_session();
        if (kDebugMode) print("Last FFplay Session: $lastFFplay");
        expect(lastFFplay, isNot(nullptr));
        ffmpeg.ffmpeg_kit_handle_release(lastFFplay);

        final lastMedia = ffmpeg
        ...
        ffmpeg.ffmpeg_kit_handle_release(lastMedia);

        // 4. List sessions
        final sessionsPtr = ffmpeg.ffmpeg_kit_get_sessions();
        int count = 0;
        ...
        final ffmpegSessionsPtr = ffmpeg.ffmpeg_kit_get_ffmpeg_sessions();
        int ffmpegCount = 0;
        if (ffmpegSessionsPtr != nullptr) {
          final list = ffmpegSessionsPtr.cast<Pointer<Void>>();
          while (list[ffmpegCount] != nullptr) {
            ffmpeg.ffmpeg_kit_handle_release(list[ffmpegCount]);
            ffmpegCount++;
          }
          ffmpeg.ffmpeg_kit_free(ffmpegSessionsPtr.cast());
        }
        ...
        // 6. Cleanup
        ffmpeg.ffmpeg_kit_handle_release(ffmpegSession);
        ffmpeg.ffmpeg_kit_handle_release(ffprobe);
        ffmpeg.ffmpeg_kit_handle_release(ffplay);
        ffmpeg.ffmpeg_kit_handle_release(media);
      });
    });
    test('FFmpegKitTest LastCompletedSession', () async {
      await using((Arena arena) async {
        ...
        final session = ffmpeg.ffmpeg_kit_execute(cmd);

        // ASSERT_NE(session, nullptr);
        expect(session, isNot(nullptr));

        // FFmpegSessionHandle last_completed = ffmpeg_kit_get_last_completed_session();
        final lastCompleted = ffmpeg.ffmpeg_kit_get_last_completed_session();
        ...
        if (lastCompleted != nullptr) {
          ...
          final state = ffmpeg.ffmpeg_kit_session_get_state(lastCompleted);
          expect(
            state,
            equals(
              ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_COMPLETED,
          ...
          ffmpeg.ffmpeg_kit_handle_release(lastCompleted);
        }
    ...
    test('FFmpegKitTest SessionProperties', () async {
      await using((Arena arena) async {
        ...
        final createTime = ffmpeg.ffmpeg_kit_session_get_create_time(session);
        // long start_time = ffmpeg_kit_session_get_start_time(session);
        final startTime = ffmpeg.ffmpeg_kit_session_get_start_time(session);
        // long end_time = ffmpeg_kit_session_get_end_time(session);
        final endTime = ffmpeg.ffmpeg_kit_session_get_end_time(session);
        // long duration = ffmpeg_kit_session_get_duration(session);
        final duration = ffmpeg.ffmpeg_kit_session_get_duration(session);
    ...
    test('FFmpegKitTest Statistics', () async {
      await using((Arena arena) async {
        ...
        final statsCount = ffmpeg.ffmpeg_kit_session_get_statistics_count(
          session,
        );
        ...
        if (statsCount > 0) {
          ...
          final stats = ffmpeg.ffmpeg_kit_session_get_statistics_at(session, 0);
          expect(stats, isNot(nullptr));

          // int frame_number = ffmpeg_kit_statistics_get_video_frame_number(stats);
          final frameNumber = ffmpeg
              .ffmpeg_kit_statistics_get_video_frame_number(stats);
          // float fps = ffmpeg_kit_statistics_get_video_fps(stats);
          final fps = ffmpeg.ffmpeg_kit_statistics_get_video_fps(stats);
          // double time = ffmpeg_kit_statistics_get_time(stats);
          final time = ffmpeg.ffmpeg_kit_statistics_get_time(stats);
          ...
          ffmpeg.ffmpeg_kit_handle_release(stats);
        }
    ...
    test('FFprobeKitTest LastSessionAliases', () async {
      await using((Arena arena) async {
        ...
        final session = ffmpeg.ffprobe_kit_execute(cmd);

        // ASSERT_NE(session, nullptr);
        expect(session, isNot(nullptr));

        // FFprobeSessionHandle last = ffprobe_kit_get_last_session();
        final last = ffmpeg.ffprobe_kit_get_last_session();
        ...
        final lastComp = ffmpeg.ffprobe_kit_get_last_completed_session();

        // EXPECT_NE(last_comp, nullptr);
        expect(lastComp, isNot(nullptr));
        if (lastComp != nullptr) {
          ffmpeg.ffmpeg_kit_handle_release(lastComp);
        }
    ...
    test('FFmpegKitTest SessionListingAliases', () async {
      ...
      await using((Arena arena) async {
        ...
        final ffmpegListPtr = ffmpeg.ffmpeg_kit_list_sessions();
        int ffmpegCount = 0;
        if (ffmpegListPtr != nullptr) {
          ...
          ffmpeg.ffmpeg_kit_free(ffmpegListPtr.cast());
        }
        ...
        final ffprobeListPtr = ffmpeg.ffprobe_kit_list_sessions();
        int ffprobeCount = 0;
        if (ffprobeListPtr != nullptr) {
          final list = ffprobeListPtr.cast<Pointer<Void>>();
          while (list[ffprobeCount] != nullptr) {
            ffmpeg.ffmpeg_kit_handle_release(list[ffprobeCount]);
            ffprobeCount++;
          }
          ffmpeg.ffmpeg_kit_free(ffprobeListPtr.cast());
        }
        ...
        final mediaListPtr = ffmpeg.media_information_kit_list_sessions();
        int mediaCount = 0;
        if (mediaListPtr != nullptr) {
          final list = mediaListPtr.cast<Pointer<Void>>();
          while (list[mediaCount] != nullptr) {
            ffmpeg.ffmpeg_kit_handle_release(list[mediaCount]);
            mediaCount++;
          }
          ffmpeg.ffmpeg_kit_free(mediaListPtr.cast());
        }
    ...
    test('FFmpegKitTest HandleManagement', () async {
      await using((Arena arena) async {
        ...
        final session = ffmpeg.ffmpeg_kit_create_session(
          toNative("-version", arena),
        );
        expect(session, isNot(nullptr));
        ...
        ffmpeg.ffmpeg_kit_handle_release(nullptr);
      });
    });
    ...
    test('StressTest SerialSyncHammer', () async {
      await using((Arena arena) async {
        ...
        for (int i = 0; i < iterations; ++i) {
          final session = ffmpeg.ffmpeg_kit_execute(
            toNative("-version", arena),
          );
          expect(session, isNot(nullptr));
          expect(
      ...
      await using((Arena arena) async {
        ...
        final tasks = List.generate(threadCount, (_) async {
          for (int i = 0; i < iterationsPerThread; ++i) {
            final session = ffmpeg.ffprobe_kit_execute(
              toNative("-version", arena),
            );
            if (session != nullptr) {
              ffmpeg.ffmpeg_kit_handle_release(session);
    ...
    test('StressTest AsyncBurstHammer', () async {
      await using((Arena arena) async {
        ...
        for (int i = 0; i < burstSize; ++i) {
          final session = ffmpeg.ffmpeg_kit_execute_async(
            cmd,
            completeCallback.nativeFunction,
            nullptr,
          );
    ...
    test('StressTest SessionHistoryConcurrency', () async {
      await using((Arena arena) async {
        ...
        final creator = () async {
          while (!stop) {
            final s = ffmpeg.ffmpeg_kit_execute_async(
              toNative("-version", arena),
              nullptr,
              nullptr,
            );
    ...
    test('StressTest FFplaySessionRecycling', () async {
      ...
      await using((Arena arena) async {
        ...
        for (int i = 0; i < iterations; ++i) {
          ...
          final session = ffmpeg.ffplay_kit_execute(cmd, 1000);
          if (session != nullptr) {
            ffmpeg.ffmpeg_kit_handle_release(session);
          }
        }
    ...
    test('StressTest MixedHammer', () async {
      ...
      await using((Arena arena) async {
        ...
        for (int i = 0; i < iterations; ++i) {
          final s1 = ffmpeg.ffmpeg_kit_execute_async(
            toNative("-version", arena),
            nullptr,
            nullptr,
          );
          final s2 = ffmpeg.ffprobe_kit_execute_async(
            toNative("-version", arena),
            nullptr,
            nullptr,
          );
          final s3 = ffmpeg.ffprobe_kit_get_media_information_async(
            toNative(getTestVideoFile(), arena),
            nullptr,
            nullptr,
          );

          if (s1 != nullptr) ffmpeg.ffmpeg_kit_handle_release(s1);
          if (s2 != nullptr) ffmpeg.ffmpeg_kit_handle_release(s2);
          if (s3 != nullptr) ffmpeg.ffmpeg_kit_handle_release(s3);
        }
      });
    });
    ...
    test('FFmpegKitConfigTest EnvironmentVariable', () async {
      await using((Arena arena) async {
        ...
        // Set a dummy env var
        final int result = ffmpeg.ffmpeg_kit_config_set_environment_variable(
          toNative("FFMPEG_KIT_TEST_VAR", arena),
          toNative("1234", arena),
        );
    ...
    test('FFmpegKitConfigTest IgnoreSignal', () async {
      await using((Arena arena) async {
        ...
        ffmpeg.ffmpeg_kit_config_ignore_signal(
          ffmpeg.FFmpegKitSignal.FFMPEG_KIT_SIGNAL_SIGPIPE,
        );
      });
    });
    ...
    test('FFmpegKitConfigTest FontDirectory', () async {
      await using((Arena arena) async {
        ...
        ffmpeg.ffmpeg_kit_config_set_font_directory_list(fontDirs, 2, nullptr);
      });
    });
    ...
    test('FFmpegKitConfigTest LogLevelToString', () async {
      await using((Arena arena) async {
        ...
        final strPtr = ffmpeg.ffmpeg_kit_config_log_level_to_string(
          ffmpeg.FFmpegKitLogLevel.FFMPEG_KIT_LOG_LEVEL_DEBUG,
        );
        ...
        ffmpeg.ffmpeg_kit_free(strPtr.cast());
      });
    });
    ...
    test('FFmpegKitConfigTest SessionStateToString', () async {
      await using((Arena arena) async {
        ...
        final strPtr = ffmpeg.ffmpeg_kit_config_session_state_to_string(
          ffmpeg.FFmpegKitSessionState.FFMPEG_KIT_SESSION_STATE_COMPLETED,
        );
    ...
    test('FFmpegKitConfigTest ArgumentsToString', () async {
      await using((Arena arena) async {
        ...
        final strPtr = ffmpeg.ffmpeg_kit_config_arguments_to_string(args, 5);
  ...
  test('FFmpegKitTest RobustnessTest', () async {
    await using((Arena arena) async {
      ...
      // 2. Get session ID
      final id = ffmpeg.ffmpeg_kit_session_get_session_id(session);
  ...
  group('CallbackTests', () {
    ...
    test('CallbackTest FFmpegAsyncExecute', () async {
      await using((Arena arena) async {
        ...
        expect(
          ffmpeg.ffmpeg_kit_session_get_session_id(session),
          equals(
            ffmpeg.ffmpeg_kit_session_get_session_id(capturer.capturedSession!),
          ),
        );
    ...
    test('CallbackTest FFmpegAsyncExecuteFull', () async {
      await using((Arena arena) async {
        ...
        final session = ffmpeg.ffmpeg_kit_execute_async_full(
          toNative(cmd, arena),
          completeCallback.nativeFunction,
    ...
    test('CallbackTest MediaInformationAsync', () async {
      ...
      await using((Arena arena) async {
        ...
        final session = ffmpeg.ffprobe_kit_get_media_information_async(
        ...
        final allPropsPtr = ffmpeg.media_information_get_all_properties_json(
          mediaInfo,
        );
        ...
        if (mediaInfo != nullptr) {
          final formatPtr = ffmpeg.media_information_get_format(mediaInfo);
          if (formatPtr != nullptr) {
            ffmpeg.ffmpeg_kit_free(formatPtr.cast());
          }
          ffmpeg.ffmpeg_kit_handle_release(mediaInfo);
        }

        mediaInfoCb.close();
        ffmpeg.ffmpeg_kit_free(allPropsPtr.cast());
        ffmpeg.ffmpeg_kit_handle_release(session);
      });
    });
    ...
    test('CallbackTest GlobalCallbacks', () async {
      ...
      await using((Arena arena) async {
        ...
        final s3 = ffmpeg.ffplay_kit_execute_async(
          toNative(
        ...
        final s4 = ffmpeg.ffprobe_kit_get_media_information_async(
          toNative(getTestVideoFile(), arena),
          nullptr,
          nullptr,
        );
    ...
    group('FFmpegKit Create Session Completer Tests', () {
      test('CallbackTest FFmpegCreateSessionWithCallbacks', () async {
        await using((Arena arena) async {
          ...
          ffmpeg.ffmpeg_kit_set_callbacks(
            session,
            nullptr,
            nullptr,
            nullptr,
      ...
      test('CallbackTest FFprobeCreateSessionWithCallbacks', () async {
        await using((Arena arena) async {
          ...
          final session = ffmpeg.ffprobe_kit_create_session_with_callbacks(
            toNative("-version", arena),
            completeCb.nativeFunction,
            nullptr,
            nullptr,
          ...
          expect(session, isNot(nullptr));
          ffmpeg.ffprobe_kit_session_execute_async(session);
      ...
      test('CallbackTest MediaInformationCreateSessionWithCallbacks', () async {
        await using((Arena arena) async {
          ...
          expect(session, isNot(nullptr));
          ffmpeg.media_information_session_execute_async(session, 5000);
