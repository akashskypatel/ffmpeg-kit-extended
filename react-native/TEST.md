...
For a real Wasm smoke test, point the consuming application's `web` or `wasm` configuration at a local directory containing the matching `ffmpegkit.mjs` and `ffmpegkit.wasm`, start the example with `npm --prefix example run web`, and use a headless Chromium session. The browser must report `crossOriginIsolated === true`; the smoke flow initializes once, repeats initialization, runs FFmpeg and FFprobe, exercises FFplay controls, and verifies that no console errors are emitted.
