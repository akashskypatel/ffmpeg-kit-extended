...
test("resolves the pinned WebAssembly bundle convention", () => {
  ...
  assert.equal(
    result.filename,
    "bundle-base-wasm-wasm32-static-small-lgpl.zip"
  );
  assert.equal(
    result.url,
...
test("rejects unsupported remote override schemes", () => {
  const { nested } = createApp({
    android: "ftp://example.com/releases/custom-bundle.aar",
  });
...
test("local-only validation rejects default and remote artifact resolution", () => {
  const { nested: defaultRoot } = createApp({});
  assert.throws(
    () =>
      resolveConfig({ appRoot: defaultRoot, platform: "web", localOnly: true }),
    /requires an explicit local bundle override/
  );

  const { nested: remoteRoot } = createApp({
    web: "https://example.com/runtime.zip",
  });
  assert.throws(
    () =>
      resolveConfig({ appRoot: remoteRoot, platform: "web", localOnly: true }),
    /rejects remote bundle overrides/
  );
});

test("local-only validation accepts a local artifact override", () => {
  const { nested } = createApp({ web: "./vendor/runtime.zip" });
  const result = resolveConfig({
    appRoot: nested,
    platform: "web",
    localOnly: true,
  });
  assert.equal(result.override.kind, "local");
});
