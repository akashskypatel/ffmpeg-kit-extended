...
Handle getFFplaySession(std::int64_t id) {
  ...
  if (handle == nullptr) throw std::runtime_error("FFplay session not found");
  using Pred = bool (*)(Handle);
...
std::string getSessionsJson(const std::string &kind) {
  ...
  if (kind == "ffmpeg") symbol = "ffmpeg_kit_get_ffmpeg_sessions";
  else if (kind == "ffprobe") symbol = "ffmpeg_kit_get_ffprobe_sessions";
  else if (kind == "ffplay") symbol = "ffmpeg_kit_get_ffplay_sessions";
  else if (kind == "media-information") symbol = "ffmpeg_kit_get_media_information_sessions";
...
std::string getLastSessionJson(const std::string &kind) {
  ...
  if (kind == "ffmpeg") symbol = "ffmpeg_kit_get_last_ffmpeg_session";
  else if (kind == "ffprobe") symbol = "ffmpeg_kit_get_last_ffprobe_session";
  else if (kind == "ffplay") symbol = "ffmpeg_kit_get_last_ffplay_session";
  else if (kind == "media-information") symbol = "ffmpeg_kit_get_last_media_information_session";
  using Fn = Handle (*)();
...
void clearSessions() { ensureInitialized(); using Fn = void (*)(); resolve<Fn>("ffmpeg_kit_clear_sessions")(); }
...
void enableDebugLog(double sessionId) { ensureInitialized(); HandleGuard h{getSession(toId(sessionId))}; if (!h.handle) throw std::runtime_error("Session not found"); using Fn = void (*)(Handle); resolve<Fn>("session_enable_debug_log")(h.handle); }
void disableDebugLog(double sessionId) { ensureInitialized(); HandleGuard h{getSession(toId(sessionId))}; if (!h.handle) throw std::runtime_error("Session not found"); using Fn = void (*)(Handle); resolve<Fn>("session_disable_debug_log")(h.handle); }
bool isDebugLogEnabled(double sessionId) { ensureInitialized(); HandleGuard h{getSession(toId(sessionId))}; if (!h.handle) return false; using Fn = bool (*)(Handle); return resolve<Fn>("session_is_debug_log_enabled")(h.handle); }
std::string getDebugLog(double sessionId) { ensureInitialized(); HandleGuard h{getSession(toId(sessionId))}; if (!h.handle) return {}; using Fn = char *(*)(Handle); return takeString(resolve<Fn>("session_get_debug_log")(h.handle)); }
void clearDebugLog(double sessionId) { ensureInitialized(); HandleGuard h{getSession(toId(sessionId))}; if (!h.handle) return; using Fn = void (*)(Handle); resolve<Fn>("session_clear_debug_log")(h.handle); }
