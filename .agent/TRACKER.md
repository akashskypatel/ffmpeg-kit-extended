...
- Native prerequisite remains frozen and independently verified at `196567dae7fd1509c33bf32081f8237596ac8e5b`: the elevated ManyLinux `FFmpegKit` checkout is clean with `HEAD == origin/dev`; the root gitlink points to the same SHA. Existing FFK23-N6 evidence remains authoritative: native gtests, Wasm Node/CTest callback tests **11/11**, ASAN/LSAN **108/111** with only the three known FFmpeg 9.0 hard-TLS failures, TSAN **108/111** with the two documented frame-data publication/lifetime warnings, and UBSAN focused **1/1** with the documented GCC 14/libstdc++ RTTI/vptr diagnostic. No sanitizer scenario was filtered or weakened.
...
- Exact-SHA closeout audit: native `HEAD == origin/dev == 196567dae7fd1509c33bf32081f8237596ac8e5b`; wrapper source `78a3e4d7e6151deb16ec6cde8857d14d31172d5b == origin/dev-wasm` at snapshot dispatch; every final workflow head matched the wrapper source SHA; the worktree was clean before the tracker-only evidence update; and the source snapshot artifact matched the source SHA. Codebase-memory coverage returned no recorded issue for the operated paths, but the changed Flutter fixture and smoke file were not tracked and Flutter/RN source metadata was stale/changed, so direct source reads and test output remain authoritative.
