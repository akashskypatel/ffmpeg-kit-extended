...
- Builder source authority at the accepted N9 handoff: `HEAD == origin/dev == aad083c3bdcf50b6047047ee915111d8bebb7cd6`; the `libs/libffmpegkit` working-tree gitlink points exactly to this SHA. The local release bundles used for wrapper validation were built from the ABI-equivalent `d654d59979c4fa8adc1a3deec0a3c2e8736b1697` source before the test-only version-unpin cleanup at `a0b9c936`.
...
| **FFK24-N10** | Resolve the FFmpeg `FrameData` TSAN publication/lifetime race | **Active — next goal; investigate the producer/consumer lifetime ordering; prior callback-suite timeout remains unresolved** |
| **FFK24-N11** | Resolve/classify GCC 14/libstdc++ UBSAN vptr diagnostics | **Open — investigation required** |
| **FFK24-N12** | Freeze clean native handoff after N7–N11 | **Blocked by N10–N11 and the affected-suite timeout; N9 has an accepted disposition** |
...
| **B5** FrameData TSAN race / callback-suite hang | Current unresolved native execution blocker | **FFK24-N10 active** — investigate the race; earlier suite timeout at `CallbackTest.GlobalCallbacks` remains evidence; no suppression or scenario weakening |
...
- Transition: FFK24-N8 is complete for the in-place ABI migration and paired local wrapper validation. N9 is completed by user acceptance; N10 is the next active investigation after the prior callback-suite timeout; N11 remains open for UBSAN classification; N12 and publication/browser closeout remain blocked.
...
- **Transition:** N9 is completed by user acceptance. FFK24-N10 is the next active goal; the existing `FrameData` TSAN report and callback-suite timeout remain unresolved and require separate investigation.
...
- The preparation-time blocker inventory above is superseded by the current ledger and FFK24-N8/FFK24-N9 transition sections: B3A is resolved, B4 is completed by user acceptance, B5 is the active N10 investigation, and B7 is handled by the user-authorized local browser gate. The only accepted production exception remains `ffigen_js: ^0.0.16-pre`.
...
N7 and N9 have explicit dispositions. N12 cannot close until N10–N11 have evidenced dispositions and the native full-suite gate is rerun without an unexplained execution blocker.
