...
| **R31-G7** | Reconcile evidence, freeze the exact wrapper source, and record the source snapshots | **Pending — local implementation and platform evidence complete; source snapshots pending** |
...
- The remaining closeout action is to verify a clean pushed wrapper SHA, dispatch the explicitly requested wrapper and `ffmpeg-kit-builders` source-snapshot workflows, and record each run ID, artifact ID, and download link below before changing R31-G7 to Complete.
